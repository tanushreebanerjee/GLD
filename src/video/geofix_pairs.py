...
def pool_mask(plane: np.ndarray, grid: int, mode: str = "max") -> torch.Tensor:
    ...
    A NON-DIVISIBLE PLANE RAISES, matching GeoFix's canonical
    `geofix.masks.pooling.pool_to_tokens`. This used `adaptive_*_pool2d` for the
    generality -- "a plane whose side is not an exact multiple of `grid` still
    pools instead of raising", and the generality is indeed free at 504/36, where
    the windows are exactly 14x14. The problem is which way the two copies
    diverged: every ANALYSIS tool refuses a non-conforming mask, while the
    TRAINING dataloader accepted one, silently, with uneven overlapping windows.
    A mask arriving at any other resolution would then raise in every scorer and
    quietly train -- the one direction that cannot be caught by looking at the
    numbers. `adaptive_*_pool2d` is kept for the pooling itself (identical output
    on the divisible case); only the silent acceptance is removed.
    """
    a = plane[None] if plane.ndim == 2 else plane
    h, w = a.shape[-2:]
    if h % grid or w % grid:
        raise ValueError(
            f"{h}x{w} does not divide into a {grid}x{grid} grid. Resampling to make "
            "it fit would smooth the mask, which is what max pooling is here to "
            "prevent (GeoFix hard rule 6; same refusal as "
            "geofix.masks.pooling.pool_to_tokens).")
    t = torch.from_numpy(np.ascontiguousarray(a)).float().div_(255.0)
    if mode == "rms":
        # The token's own RMSE -- square, mean, root. For an ORACLE plane under an
        # L2 criterion this is the aggregation the criterion implies, where MAX is
        # an L-infinity one. See `geofix.masks.pooling` for why this does not
        # violate hard rule 6 (that rule is about DETECTION by a deployable mask)
        # and for the measurement that says the pooling axis matters ~100x more
        # than the L1-vs-L2 norm axis.
...
class GeoFixPairs(Dataset):
    ...
    def __init__(self, manifest: str | pathlib.Path, *, mask_types=None,
        ...
        if pooling not in POOLING_MODES:
            ...
            # `rms` and `max_arearef_rms` are the L2-oracle pair -- see
            # POOLING_MODES and `geofix.masks.pooling`.
            raise ValueError(
        ...
        #: Counted, not silently absorbed: frames where the area match had no
        #: solution at ANY exponent. Read it after an epoch; a nonzero count
        #: means the control is not exactly matched on those frames.
